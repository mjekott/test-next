...
const Button = <E extends React.ElementType = typeof defaltElement>({
  children,
  size = 'md',
  variant = 'solid',
  className,
  ...
  const Component = as || defaltElement;
  return (
    <Component
      className={clsx(
        variants[variant],
    ...
    >
      {loading ? (
        <svg
          role="status"
          className={clsx(
            'mr-2 w-6 h-6 text-gray-200 animate-spin  fill-white'
          )}
          viewBox="0 0 100 101"
          fill="none"
